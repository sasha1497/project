...
export class RazorpayController {
  ...
  async createSubscriptionOrder(@Body() data: any) {
    ...
    const { user_id, plan_id, country, currency, order_amount, receipt } = data;

    console.log('📥 Request:', { user_id, plan_id, country, currency, order_amount, receipt });

    // 1. Create Razorpay order using frontend-provided amount and currency
    const order = await this.razorpayService.createOrder(order_amount, currency, receipt);
    ...
    return {
      ...
      final_price: { order_amount, currency },
    };
  }
}
